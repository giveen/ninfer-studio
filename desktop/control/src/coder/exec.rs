...
static DESTRUCTIVE_PATTERNS: LazyLock<Vec<(regex::Regex, &'static str)>> = LazyLock::new(|| {
    let cases: &[(&str, &'static str)] = &[
        ...
        (
            r"(?i)\b(curl|wget|fetch)\b(?s:.)*?\|\s*(ba)?sh\b",
...
pub async fn exec(
    ...
    let (sandboxed, sandbox_binds) = {
        let c = state.config.read().await;
        (
            c.coder_sandbox && crate::sandbox::available() && !root.as_os_str().is_empty(),
            c.sandbox_binds.clone(),
        )
    };
        ...
        .unwrap_or(false)
    {
        ...
        return Ok(Json(
            json!({"jobId": id, "started": true, "sandboxed": sandboxed}),
        ));
    }
...
async fn drain_bg_job(
    job: std::sync::Arc<BgJob>,
    mut child: crate::sandbox::ExecChild,
    ...
    st.stdout = o;
    st.stderr = if killed && e.is_empty() {
        "killed".to_string()
    } else {
        e
    };
...
mod tests {
    ...
    fn secret_env_var_detection_is_case_insensitive_and_scoped() {
        for name in [
            ...
            assert!(
                crate::sandbox::is_secret_env_var(name),
                "expected {name} to be flagged as a secret"
            );
        }
        for name in ["PATH", "HOME", "LANG", "TERM", "PWD", "SHELL", "USER"] {
            assert!(
                !crate::sandbox::is_secret_env_var(name),
                "expected {name} to NOT be flagged as a secret"
            );
        }
    }
    ...
    async fn bg_job_round_trip() {
        ...
        let r = exec(
            ws(),
            Json(json!({"command": "echo bg-hi", "background": true})),
        )
        .await
        .unwrap()
        .0;
        ...
        let r2 = exec(
            ws(),
            Json(json!({"command": "sleep 30", "background": true})),
        )
        .await
        .unwrap()
        .0;
    ...
    async fn sandbox_set_get_persists_to_config() {
        ...
        let r = sandbox_set(
            w(),
            Json(json!({"enabled": true, "sandboxBinds": ["/mnt/models"]})),
        )
        .await;
        assert_eq!(r["enabled"], true);
        assert_eq!(r["sandboxBinds"][0], "/mnt/models");
        let cfg = serde_json::from_str::<Value>(
