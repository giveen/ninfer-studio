...
pub fn build_serve_args(p: &EngineProfile, port: u16) -> Vec<String> {
    ...
    kv(
        &mut a,
        "--model-id",
        &p.model_id.clone().unwrap_or_default(),
    );
    kv(
        &mut a,
        "--chat-template",
        &p.chat_template.clone().unwrap_or_default(),
    );
    kv(
        &mut a,
        "--max-context",
        &p.max_context.map(|v| v.to_string()).unwrap_or_default(),
    );
    match &p.kv_capacity {
        None => {}
    ...
    kv(
        &mut a,
        "--max-concurrency",
        &p.max_concurrency.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--max-pending-requests",
        &p.max_pending_requests
            .map(|v| v.to_string())
            .unwrap_or_default(),
    );
    kv(
    ...
    kv(
        &mut a,
        "--prefill-chunk",
        &p.prefill_chunk.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--log-stats-interval-ms",
        &p.log_stats_interval_ms
            .map(|v| v.to_string())
            .unwrap_or_default(),
    );
    kv(
        &mut a,
        "--log-level",
        &p.log_level.clone().unwrap_or_default(),
    );
    kv(
        &mut a,
        "--device",
        &p.device.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--context-cost-presets",
        &p.context_cost_presets.clone().unwrap_or_default(),
    );
    kv(
        &mut a,
        "--max-request-mib",
        &p.max_request_mib.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--media-cache-mib",
        &p.media_cache_mib.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--media-live-mib",
        &p.media_live_mib.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--media-preprocess-threads",
        &p.media_preprocess_threads
            .map(|v| v.to_string())
            .unwrap_or_default(),
    );
    kv(
        &mut a,
        "--request-log-jsonl",
        &p.request_log_jsonl.clone().unwrap_or_default(),
    );
    kv(
        &mut a,
        "--response-store-max-records",
        &p.response_store_max_records
            .map(|v| v.to_string())
            .unwrap_or_default(),
    );
    kv(
    ...
    kv(
        &mut a,
        "--kv-dtype",
        &p.kv_dtype.clone().unwrap_or_default(),
    );
    ...
    flag(
        &mut a,
        "--preserve-thinking",
        p.preserve_thinking == Some(true),
    );
    kv(
        &mut a,
        "--temperature",
        &p.temperature.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--top-p",
        &p.top_p.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--top-k",
        &p.top_k.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--min-p",
        &p.min_p.map(|v| v.to_string()).unwrap_or_default(),
    );
    kv(
        &mut a,
        "--presence-penalty",
    ...
    kv(
        &mut a,
        "--seed",
        &p.seed.map(|v| v.to_string()).unwrap_or_default(),
    );
...
mod parity {
    ...
    fn serve_args_match_fixture() {
        ...
        let expected: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(dir.join("expected-args.json"))
                .expect("read expected-args.json"),
        )
